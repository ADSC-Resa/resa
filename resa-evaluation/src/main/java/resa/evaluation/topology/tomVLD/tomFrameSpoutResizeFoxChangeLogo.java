 ...
 */
public class tomFrameSpoutResizeFoxChangeLogo extends BaseRichSpout {
    ...
    long openTimeStamp;
    long firstChangeTimeMS;
    boolean firstChanged;
    long secondChangeTimeMS;
    ...
    public void open(Map map, TopologyContext topologyContext, SpoutOutputCollector spoutOutputCollector) {
        ...
        firstChangeTimeMS = ConfigUtil.getInt(map, "firstChangeTimeMS", 60000);
        secondChangeTimeMS = ConfigUtil.getInt(map, "secondChangeTimeMS", 600000);
    }
    ...
    public void nextTuple() {
        ...
        if (firstChanged == false && now - openTimeStamp > firstChangeTimeMS){
            collector.emit(SIGNAL_STREAM, new Values("2nd-group"), now);
            firstChanged = true;
        }

        if (secondChanged == false && now - openTimeStamp > secondChangeTimeMS){
            collector.emit(SIGNAL_STREAM, new Values("1st-group"), now);
            secondChanged = true;
        }
...
//    }
    ...
    public void declareOutputFields(OutputFieldsDeclarer outputFieldsDeclarer) {
        ...
        outputFieldsDeclarer.declareStream(SIGNAL_STREAM, new Fields(FIELD_SIGNAL_TYPE));
    }
